...
speaker_model_Ln <- 
  nsp_vows_lob %>% group_by(Talker) %>% train_models()
gender_model_Ln <-
  nsp_vows_lob %>% group_by(Sex) %>% train_models()
dialect_model_Ln <-
  nsp_vows_lob %>% group_by(Dialect) %>% train_models()
genderByDialect_model_Ln <-
  nsp_vows_lob %>% group_by(Sex, Dialect) %>% train_models()
marginal_model_Ln <-
  nsp_vows_lob %>% ungroup() %>% train_models()
...
class_hz <-
  map2(c('talker',
       ...
       list(speaker_model,
            ...
            marginal_model),
       ~ classify_vowels(nsp_vows, .y) %>% mutate(grouping = .x)) %>%
  bind_rows() %>%
  filter(posterior_choice) %>%
  mutate(correct = Vowel == model)
...
class_Ln <-
  map2(c('talker',
       ...
       list(speaker_model_Ln,
            ...
            genderByDialect_model_Ln,
            marginal_model_Ln),
       ~ classify_vowels(nsp_vows_lob, .y) %>% mutate(grouping = .x)) %>%
  bind_rows() %>%
  filter(posterior_choice) %>%
  mutate(correct = Vowel == model)
...
model_names <- data_frame(grouping = c('marginal',
                                       'dialect',
                                       'gender',
                                       'genderByDialect',
                                       'talker'),
