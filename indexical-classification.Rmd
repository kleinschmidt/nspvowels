...
true_group_class_subsampled <-
  ...
  filter(group == group_model,
         grouping != 'Marginal') %>%
  group_by(dataset, grouping, subsample_size, Talker, group, Vowel, Token) %>%
  normalize_probability('lhood') %>%
  mutate(group_is = 'Known')

## check posterior
true_group_class_subsampled %>%
  group_by(dataset, grouping, subsample_size, Talker, Vowel, Token) %>%
...
## check posteriors are correct
## FAILS: NAs in some lhoods
vowel_class_all %>%
  group_by(dataset, grouping, group_is, subsample_size, Talker, Vowel, Token) %>%
  summarise(n_choice = sum(posterior_choice),
            sum_post = sum(posterior)) %$%
  assert_that(all(n_choice == 1),
              all.equal(sum_post, rep(1, length(sum_post))))
...
vowel_class_all %>%
  ...
  group_by(grouping, group_is, dataset, subsample_size, group) %>%
  tally()

## joint_class_subsampled
bad <- vowel_class_all %>%
  filter(is.na(posterior)) %>%
  group_by(dataset, subsample_size, grouping, Talker, Vowel, Token) %>%
  summarise() %>%
  left_join(joint_class_subsampled)

okay <- joint_class_subsampled %>% anti_join(bad)

## problem is that there're some places where all the GROUP models have
## -Inf log posterior, which gets turned into NaN
bad %>%
  group_by(grouping, dataset, subsample_size, group, Talker,
           Vowel, Token, vowel_model) %>%
  summarise_each(funs(lp = log_sum_exp, length, min, max),
                 log_posterior) %>%
  summarise(bad=any(!is.finite(lp))) %>%
  ungroup() %>% summarise(all(bad))
...
```{r dialect-vowel-spaces}

nsp_vows_lob %>%
  group_by(Dialect, Vowel) %>%
  summarise_each(funs(mean), F1, F2) %>%
  ggplot(aes(x=F2, y=F1, color=Dialect, label=Vowel)) +
  geom_text() +
...
data('clopper_competitors', package='nspvowels')

vowel_class_competitors <-
  vowel_class_all %>%
  select(-Dialect) %>%
  left_join(nsp_vows %>% group_by(Talker, Dialect) %>% summarise(), by='Talker') %>%
  inner_join(clopper_competitors, by = c('Vowel', 'Dialect'))
  

vowel_class_competitors %>%
  filter(group_is == 'Known' | grouping == 'Marginal',
         subsample_size == 7 | grouping == 'Dialect',
         vowel_model == Competitor) %>%
  group_by(dataset, grouping, Dialect, Vowel, vowel_model, Talker) %>%
  summarise(confusions = mean(posterior_choice)) %>%
  mutate(pair = paste(Vowel, vowel_model, sep=' -> ')) %>%
  ggplot(aes(x=pair, y=confusions, color=grouping, fill=grouping)) +
  geom_bar(stat='summary', fun.y = mean, position=pd()) +
  geom_linerange(stat='summary', fun.data=mean_cl_boot, position=pd()) +
  facet_grid(dataset~Dialect, space='free', scales='free') +
  ggtitle('Socio-linguistically predicted confusions (7 talker subsample, group known)') +
  labs(y = 'Proportion error occurred', x = 'Error')

## there's something very odd: this is showing average accuracy < 20% for normalized
## North eh grouped by Sex.
vowel_class_competitors %>%
  filter(group_is == 'Known' | grouping == 'Marginal',
         subsample_size == 7 | grouping == 'Dialect',
         vowel_model == Vowel) %>%
  group_by(dataset, grouping, Dialect, Vowel, Talker) %>%
  summarise(accuracy = mean(posterior_choice)) %>%
  ggplot(aes(x=Vowel, y=accuracy, color=grouping, fill=grouping)) +
  geom_bar(stat='summary', fun.y=mean, position=pd()) +
  geom_linerange(stat='summary', fun.data=mean_cl_boot, position=pd()) +
  facet_grid(dataset~Dialect, space='free', scales='free')
...
vowel_acc_all_with_sizes %>%
  filter(grouping == 'Sex',
         subsample_size == 7,
         dataset == 'Lobanov Normalized') %>%
  arrange(Talker, Vowel, Token) %>% ## MISSING TOKENS WTF
  group_by(Talker, Vowel, group_is) %>% tally()
                                        # the problem is with group_is == "known"
                                        # probably at some point a join or filter went bad. t
...
# problem is with known, subsampled. ############################
vowel_class_all %>%
  filter(grouping == 'Sex',
         dataset == 'Lobanov Normalized') %>%
  group_by(Vowel, Token, subsample_size, group_is, Talker) %>%
  summarise(x = any(posterior_choice)) %>%
  group_by(subsample_size, group_is) %>%
  summarise(all(x), mean(x))
...
vowel_class_competitors %>%
  ...
  group_by(Talker, Vowel, Competitor, group_is) %>% tally()
...
vowel_class_competitors %>%
  ...
  group_by(dataset, grouping, Dialect, Vowel, Talker) %>%
  summarise(accuracy = mean(posterior_choice)) %>%
  spread(Vowel, accuracy) %>% knitr::kable()
