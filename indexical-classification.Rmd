...
```{r fit-talker-data, cache=TRUE, dependson=-1}
...
```{r talker-class, cache=TRUE, dependson=-1}
...
Here we evaluate how informative indexical variables of gender and age are for classifying stops as voiced and unvoiced. The data comes from the Buckeye corpus of conversational speech, with VOTs for word-initial voiced and voiceless stops extracted by Wedel (DDDD).
...
Classifying stops as voiced or unvoiced is not substantially improved by knowing the age or sex of a talker, above and beyond the marginal distributions. However, using a talker's own distributions produces a slight but reliable improvement in accuracy over any of the group-level/marginal cue distributions (even with cross-validation). This shows that, while there is _some_ talker-level variability in VOT, it is not systematically predicted by any of the socio-indexical grouping factors we examined here. The case of sex is particularly informative: knowing a talker's sex provides a big boost in speech recognition for vowels, but no benefit for stop consonant voicing.
