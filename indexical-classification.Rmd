...
There might still be effects for particular vowels though (see below). My summary of these is that there's something to `ae` and `eh`, but it's not super consistent. I think it's quite likely that any of these subtle effects are going to depend on the size of the group that's subsampled (since the whole goal of that is to increase variance in the estimates to comparable levels).

On top of that, effects for single categories over all groups are not really that informative anyway. It's probably going to be more informative to look at particular dialect groups in particular, given that there are (in principle) particular vowel changes in each group.
...
vot_known_acc_all %>%
  ...
  do({ daver::boot_ci(., function(d,i) mean(d$talker_advantage[i]), h0=0) }) %>%
  knitr::kable(digits=2)
