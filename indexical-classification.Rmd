...
d <- index_models$trained[[1]]$data_train[[1]]
...
#' @return A tbl with \code{n} levels of \code{group} randomly sampled from
...
sample_n_groups <- function(tbl, group, n) {
  ...
  tbl %>%
    ...
    summarise() %>%
    sample_n(n) %>%
    left_join(tbl, by=group) %>%
    group_by_(.dots=tbl_groups)
}
  
sample_n_groups(d, 'Talker', 4)
...
train_models_indexical_subsample_holdout <- function(d, groups, n_subsample,
  ...
  d %>%
    ...
    mutate(data_train = map(data_train, . %>% sample_n_groups(holdout, n_subsample)),
...
trained <- train_models_indexical_subsample_holdout(nsp_vows, 'Sex', 4)
