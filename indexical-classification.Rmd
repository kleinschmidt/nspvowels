...
index_models <-
  ...
  mutate(data = map(data, . %>% mutate(Marginal='all',
                                       Dialect_Sex = paste(Dialect, Sex, sep='_')))) %>%
...
```{r compute-joint-posteriors, cache=TRUE, dependson=c(-1,-2,-3)}
...
The real question is how much performance improves when you _know_ the right group. Again, it's important to evaluate this using cross validation.
...
true_group_class <-
  ...
  mutate(true_group_posteriors = map2(trained, grouping, vowel_post_true_group)) %>%
  unnest(map2(true_group_posteriors, grouping, ~ rename_(.x, 'group'=.y))) %>%
  mutate(grouping = paste0(grouping, '_true')) %>%
  rename(vowel_model = model)
...
These show the _marginal_ probability of categorizing tokens, which is computed by marginalizing out group (e.g., averaging the categorization probabilities under each group, weighed by the probability of that group given all the tokens).

### Vowel confusion matrices

Vowel confusion matrices, split out by grouping factor (Dialect or Sex) and subgroups. 
...
marginal_vowel_class %>%
  ...
  facet_grid(dataset ~ grouping+group)

```

Compare with when the true group for each talker is used:

```{r true-group-vowel-confusions, fig.width=10, cache=TRUE}

true_group_class %>%
  group_by(grouping, dataset, group, Talker, Vowel, vowel_model) %>%
  summarise(posterior = mean(posterior)) %>%
  mutate(vowel_model = factor(vowel_model, levels = levels(Vowel))) %>% 
  ggplot(aes(x=Vowel, y=vowel_model, fill=posterior)) +
  geom_tile() + coord_equal() +
...
Average probability of correct recognition when jointly inferring 
...
marginal_vowel_class %>%
  ...
  group_by(grouping, dataset, group, Talker, Vowel, Token) %>%
  filter(posterior == max(posterior)) %>%
  mutate(accuracy = vowel_model == Vowel) %>%
  group_by(grouping, dataset, group, Talker, Vowel) %>%
  summarise(accuracy = mean(accuracy)) %>%
  ggplot(aes(x=Vowel, y=accuracy, color=Vowel, fill=Vowel, group=grouping)) +
