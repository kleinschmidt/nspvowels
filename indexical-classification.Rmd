...
vot_marginal_class %>%
  group_by(grouping, place, Talker, word, Token) %>%
  summarise(any_choice = any(posterior_choice)) %>%
  summary()
...
vot_group_class %>%
  filter(group == group_model) %>%
  group_by(place, grouping, group) %>%
  summarise(n(),
            accuracy = mean(posterior_choice))

## aggregate over place of articulation
vot_group_class %>%
  group_by(grouping, group, Talker, group_model) %>%
  summarise(log_posterior = sum(log_posterior)) %>%
  mutate(log_posterior = log_posterior - log_sum_exp(log_posterior),
         posterior = exp(log_posterior),
         posterior_choice = posterior == max(posterior)) %>%
  filter(group == group_model) %>%
  ggplot(aes(x=grouping, y=posterior)) +
