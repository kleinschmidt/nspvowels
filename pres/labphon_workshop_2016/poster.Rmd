...
* Can think of this as a statistical inference problem: infer current talker's
  cue distributions.
* Combine experience with current talker with prior experience with other talkers
* Structure in talker variability determines how best to use prior experience.
* A good place to look for structure: socio-indexical variables like gender,
  dialect, etc.
  
* Question: how much do socio-indexical grouping variables tell us about a
  talker's phonetic cue distributions? 
* Focus on dialect. Compare to sex and talker identity
* Structure could vary based on how cues are represented, so also compare raw
  formant frequencies (Hz) and Lobanov normalized formants (z-score within
  talker) which removes differences in overall average F1 and F2 across all
  vowels (e.g., due to differnt vocal tract sizes).
...
Marginal (all tokens from all talkers), or conditioned on talker, sex, dialect, or dialect and sex together.
...
Note: a variable can be informative but not useful, if variation doesn't lead to
confusion
...
Also useful for vowel recognition, but only for some dialect-vowel combinations

# Conclusions

Quantified how much listeners can benefit from tracking cue distributions conditioned on different socio-indexical variables

Found that dialect is informative and useful, at an intermediate level,
...
vowel_data <- data_frame(cues_long = c('Un-normalized F1xF2', 'Lobanov Normalized F1xF2'),
                         cues = c('Raw (Hz)', 'Lob. Norm.'),
...
p_vow_group_hz <- vowel_data_grouped_long %>%
  filter(cues_long == 'Un-normalized F1xF2') %>%
...
p_vow_group_lob <- vowel_data_grouped_long %>%
  filter(str_detect(cues_long, 'Lobanov')) %>% 
...
```{r vowel-vot-kl-plot, fig.width=5, fig.height=4.2}
...
vowel_kl %>%
  ...
  geom_pointrange(stat='summary', fun.data=mean_cl_boot,
                  position = position_dodge(w=0.5)) +
  facet_grid(cues ~ Vowel, scales='free', space='free') +
  labs(x = '',
       y = 'KL Divergence of cue distributions\nfrom marginal (bits)') +
...
vowel_kl %>%
  ...
  geom_text(data = vowel_kl %>%
               filter(grouping == 'Dialect', str_detect(cues, 'Lob')) %>%
               arrange(desc(KL)) %>%
               head(5),
...
vowel_acc_by_talker %>%
  ...
  geom_pointrange(data=vowel_acc_by_talker_summary,
                  stat='identity',
